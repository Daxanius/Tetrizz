...
void TetrizzScreen::OnKeyDownEvent(SDL_Keycode key)
{
  ...
  case SDLK_SPACE:
  {
    int multiplier{ int(m_PlayfieldPtr->GetQuickPlacePosition().y - m_PlayfieldPtr->GetState()->GetTetrominoPosition().y) };
...
void TetrizzScreen::PlaceTetromino()
{
  ...
  case 1:
    ...
    intensity = 1;
    AddToScore(int(intensity * (intensity * LINE_CLEAR_SCORE)));
    break;
  case 2:
...
void TetrizzScreen::DrawSaved(const Point2f& position)
{
  ...
  {
    Rectf outlineRect{
      sourceRect.left + ceilf(lineThickness / 2.f),
      sourceRect.top + ceilf(lineThickness / 2.f),
      sourceRect.width + lineThickness,
      sourceRect.height + lineThickness
    };
